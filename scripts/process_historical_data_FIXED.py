...
sys.path.append('/home/matias/nowcasting')
...
# Nuevos componentes
from src.models.advanced_nowcast_predictor import AdvancedNowcastPredictor
from src.models.uncertainty_modeling import UncertaintyModeling
from src.models.shape_evolution_model import StormCellEvolutionModel
from src.evaluation.nowcasting_evaluator import NowcastingEvaluator

# Configuración de logging
log_dir = '/home/matias/nowcasting/logs'
...
def parse_arguments():
    ...
    parser.add_argument('--data_dir', type=str, 
                        default='/home/matias/nowcasting/data/raw',
                        help='Directory with GLM data files')
    
    parser.add_argument('--output_dir', type=str, 
                        default='/home/matias/nowcasting/outputs/historical',
    ...
    # En la función parse_arguments, añade estos nuevos argumentos
    parser.add_argument('--use-advanced-models', action='store_true',
                        help='Use advanced machine learning models for predictions')

    parser.add_argument('--show-uncertainty', action='store_true',
                        help='Show uncertainty information in visualizations')

    parser.add_argument('--predict-shape', action='store_true',
                        help='Predict shape evolution of storm cells')

    parser.add_argument('--evaluate', action='store_true',
                        help='Perform detailed evaluation of predictions')
...
def main():
    ...
    cell_nowcaster = FlashCellNowcaster(forecast_minutes=args.forecast_minutes, max_lags=1)
    ...
    # Inicializar componentes adicionales si se solicitan
    advanced_predictor = None
    uncertainty_model = None
    shape_model = None
    evaluator = None

    if args.use_advanced_models:
        logger.info("Initializing advanced predictor")
        advanced_predictor = AdvancedNowcastPredictor(model_type='random_forest')
        # Cargar modelos pre-entrenados si existen
        model_dir = os.path.join(args.output_dir, 'models')
        if os.path.exists(model_dir):
            model_paths = {}
            import glob
            for model_name in ['position_lat', 'position_lon', 'intensity', 'area']:
                model_files = glob.glob(os.path.join(model_dir, f'nowcast_{model_name}_random_forest_*.joblib'))
                if model_files:
                    model_paths[model_name] = model_files[0]
            if model_paths:
                logger.info(f"Loading {len(model_paths)} pre-trained models")
                advanced_predictor.load_models(model_paths)

    if args.show_uncertainty:
        logger.info("Initializing uncertainty modeling")
        uncertainty_model = UncertaintyModeling()

    if args.predict_shape:
        logger.info("Initializing shape evolution model")
        shape_model = StormCellEvolutionModel()

    if args.evaluate:
        logger.info("Initializing nowcasting evaluator")
        eval_dir = os.path.join(output_dir, 'evaluation')
        os.makedirs(eval_dir, exist_ok=True)
        evaluator = NowcastingEvaluator(output_dir=eval_dir)
    ...
    while current_time < end_dt:
        ...
        save_clusters_csv(flash_df_with_clusters, diagnostics_dir, window_end)
        ...
        if 'timestamp' not in cells_gdf.columns:
            cells_gdf['timestamp'] = window_end  # Usar el tiempo de fin de la ventana

        # Realizar seguimiento de celdas
        tracked_cells = cell_tracker.track_cells(cells_gdf, window_end)
        
        if 'timestamp' not in tracked_cells.columns:
            tracked_cells['timestamp'] = window_end
        
        # Realizar predicción con el modelo básico
        predictions_df = cell_nowcaster.predict_cells(tracked_cells, cell_tracker.tracked_cells)
        
        # Usar modelos avanzados si están disponibles y habilitados
        if args.use_advanced_models and advanced_predictor and len(all_tracked_gdfs) >= 2:
            try:
                # Preparar datos históricos (últimas 3 ventanas como máximo)
                train_data = []
                for i in range(max(0, len(all_tracked_gdfs)-3), len(all_tracked_gdfs)):
                    if not all_tracked_gdfs[i].empty:
                        train_data.append(all_tracked_gdfs[i])
                
                # Entrenar el modelo si hay suficientes datos
                if len(train_data) >= 2:
                    advanced_predictor.train(train_data)
                    
                    # Preparar datos históricos con timestamp
                    train_data_with_time = []
                    for i in range(max(0, len(all_tracked_gdfs)-3), len(all_tracked_gdfs)):
                        if not all_tracked_gdfs[i].empty:
                            df_copy = all_tracked_gdfs[i].copy()
                            if 'timestamp' not in df_copy.columns:
                                df_copy['timestamp'] = all_windows[i][1]  # Usar tiempo de fin de ventana
                            train_data_with_time.append(df_copy)

                    # Usar estos datos para entrenamiento
                    if len(train_data_with_time) >= 2:
                        advanced_predictor.train(train_data_with_time)

                    # Generar predicciones avanzadas
                    advanced_predictions = []
                    for track_id in tracked_cells['track_id'].unique():
                        # Obtener datos históricos de este track
                        track_history = []
                        for gdf in all_tracked_gdfs:
                            if not gdf.empty and track_id in gdf['track_id'].values:
                                track_history.append(gdf[gdf['track_id'] == track_id].iloc[0])
                        
                        if len(track_history) >= 2:
                            # Generar predicción para los próximos 10, 20 y 30 minutos
                            for minutes in [15, 30, 45]:
                                pred = advanced_predictor.predict_next_state(track_history[-1], time_delta_minutes=minutes)
                                if pred:
                                    advanced_predictions.append(pred)
                    
                    # Si hay predicciones avanzadas, usarlas en lugar de las básicas
                    if advanced_predictions:
                        predictions_df = pd.DataFrame(advanced_predictions)
                        logger.info(f"Generadas {len(predictions_df)} predicciones con modelo avanzado")
            except Exception as e:
                logger.error(f"Error al usar modelo avanzado: {e}")
                # En caso de error, se mantienen las predicciones del modelo básico
        
        # Agregar incertidumbre si está habilitada
        if args.show_uncertainty and uncertainty_model and not predictions_df.empty:
            try:
                # Agregar incertidumbre a las predicciones
                predictions_df = uncertainty_model.add_uncertainty_to_predictions(predictions_df)
                logger.info("Agregada información de incertidumbre a las predicciones")
            except Exception as e:
                logger.error(f"Error al agregar incertidumbre: {e}")
        
        # Predicción de evolución de forma si está habilitada
        if args.predict_shape and shape_model and not predictions_df.empty and len(all_cell_gdfs) >= 2:
            try:
                # Preparar datos por track
                features_by_track = {}
                for track_id in tracked_cells['track_id'].unique():
                    track_cells = []
                    for i, gdf in enumerate(all_cell_gdfs):
                        if not gdf.empty and 'track_id' in gdf.columns and track_id in gdf['track_id'].values:
                            cell = gdf[gdf['track_id'] == track_id].iloc[0].copy()
                            # Asegurarse de que timestamp esté presente
                            if 'timestamp' not in cell or pd.isna(cell['timestamp']):
                                cell['timestamp'] = all_windows[i][1]  # Usar tiempo de fin de ventana
                            track_cells.append(cell)
                    
                    if len(track_cells) >= 2:
                        # Asegurarse de que track_id está presente en cada celda
                        cell_df = pd.DataFrame([cell for cell in track_cells if 'cell_id' in cell])
                        if not cell_df.empty:
                            features_by_track[track_id] = shape_model.extract_cell_features(cell_df)

                # Entrenar modelo de evolución si hay suficientes datos
                if features_by_track:
                    shape_model.fit(features_by_track)
                    
                    # Generar predicciones de evolución para cada celda actual
                    shape_predictions = {}
                    for _, cell in tracked_cells.iterrows():
                        if 'track_id' in cell and cell['track_id'] in features_by_track:
                            # Extraer características
                            cell_features = shape_model.extract_cell_features(pd.DataFrame([cell])).iloc[0]
                            
                            # Predecir evolución (3 pasos = 30 minutos)
                            evolution = shape_model.predict_evolution(cell_features, timesteps=3)
                            
                            if evolution:
                                shape_predictions[cell['track_id']] = evolution
                    
                    # Guardar predicciones de evolución
                    if shape_predictions:
                        timestamp_str = window_end.strftime('%Y%m%d_%H%M%S')
                        shape_file = os.path.join(predictions_dir, f'shape_evolution_{timestamp_str}.json')
                        with open(shape_file, 'w') as f:
                            # Convertir keys a string para serialización JSON
                            json_shape_predictions = {}
                            for k, v in shape_predictions.items():
                                json_shape_predictions[str(k)] = v
                            json.dump(json_shape_predictions, f)
                        logger.info(f"Generadas predicciones de evolución de forma para {len(shape_predictions)} celdas")
            except Exception as e:
                logger.error(f"Error al predecir evolución de forma: {e}")
                # Imprimir el traceback completo para debugging
                import traceback
                logger.error(traceback.format_exc())        
        
        # Crear geometrías para predicciones
        ...
        if args.visualize and visualizer:
            ...
            m = visualizer.create_interactive_map(
                ...
                uncertainty_data=predictions_df if args.show_uncertainty else None
            )
    ...
    # INICIALIZAR VARIABLES PARA PREDICCIONES CONSOLIDADAS
    consolidated_predictions = []
    last_cells = None
    
    # Preparar datos para predicciones consolidadas
    if all_tracked_gdfs and len(all_tracked_gdfs) > 0:
        # Obtener la última ventana con datos válidos
        for i in range(len(all_tracked_gdfs) - 1, -1, -1):
            if not all_tracked_gdfs[i].empty:
                last_cells = all_tracked_gdfs[i]
                logger.info(f"Using window {i} with {len(last_cells)} cells for consolidated predictions")
                break
    
    # Generar predicciones consolidadas si tenemos datos válidos
    if last_cells is not None and not last_cells.empty:
        logger.info("Generating consolidated predictions...")
        
        # Usar modelo avanzado si está disponible
        if args.use_advanced_models and advanced_predictor and len(all_tracked_gdfs) >= 2:
            try:
                # Preparar datos históricos para entrenamiento
                train_data = []
                for i in range(max(0, len(all_tracked_gdfs)-3), len(all_tracked_gdfs)):
                    if i < len(all_tracked_gdfs) and not all_tracked_gdfs[i].empty:
                        gdf_copy = all_tracked_gdfs[i].copy()
                        if 'timestamp' not in gdf_copy.columns:
                            gdf_copy['timestamp'] = all_windows[i][1]
                        train_data.append(gdf_copy)
                
                if len(train_data) >= 2:
                    # Entrenar el modelo avanzado
                    advanced_predictor.train(train_data)
                    
                    # Generar predicciones para cada celda activa
                    for _, cell in last_cells.iterrows():
                        track_id = cell['track_id']
                        
                        # Extraer historial de esta celda
                        track_history = []
                        for i, gdf in enumerate(all_tracked_gdfs):
                            if not gdf.empty and 'track_id' in gdf.columns and track_id in gdf['track_id'].values:
                                track_info = gdf[gdf['track_id'] == track_id].iloc[0].copy()
                                if 'timestamp' not in track_info:
                                    track_info['timestamp'] = all_windows[i][1]
                                track_history.append(track_info)
                        
                        # Generar predicciones a diferentes tiempos
                        if len(track_history) >= 2:
                            for minutes in [15, 30, 45]:
                                pred = advanced_predictor.predict_next_state(track_history[-1], time_delta_minutes=minutes)
                                if pred:
                                    ...
                                    pred['forecast_minutes'] = minutes
                                    consolidated_predictions.append(pred)
                    
                    logger.info(f"Generated {len(consolidated_predictions)} advanced predictions")
        ...
        # Si no hay predicciones avanzadas, usar el modelo básico
        if not consolidated_predictions and cell_tracker.tracked_cells:
            logger.info("Using basic nowcasting model for consolidated predictions...")
            
            for _, cell in last_cells.iterrows():
                track_id = cell['track_id']
                if track_id in cell_tracker.tracked_cells:
                    track_history = cell_tracker.tracked_cells[track_id]
                    if len(track_history) >= cell_nowcaster.min_history_points:
                        for minutes in [15, 30, 45]:
                            temp_nowcaster = FlashCellNowcaster(forecast_minutes=minutes)
                            pred = temp_nowcaster._predict_cell_movement(track_history)
                            if pred:
                                pred['track_id'] = track_id
                                pred['last_cell_id'] = cell['cell_id']
                                pred['forecast_minutes'] = minutes
            ...
            logger.info(f"Generated {len(consolidated_predictions)} basic predictions")
    
    # Crear visualización consolidada si está habilitada
    if args.visualize and visualizer and all_tracked_gdfs:
        logger.info("Creating consolidated visualization with tracks and predictions...")
        ...
        for i, window in enumerate(all_windows):
            if i < len(all_tracked_gdfs) and not all_tracked_gdfs[i].empty:
                cells_history.append(all_tracked_gdfs[i])
                timestamps.append(window[1])  # Usar tiempo final de cada ventana
        
        # 2. Verificar que tenemos datos válidos
        if not cells_history:
            logger.warning("No hay datos históricos válidos para visualización consolidada")
        elif len(cells_history) < 2:
            logger.warning("Datos insuficientes para generar mapa consolidado")
        else:
            # 3. Usar el último conjunto de celdas para predicciones
            last_cells = cells_history[-1]
            
            # Verificar que last_cells tiene datos válidos
            if last_cells.empty:
                logger.warning("El último conjunto de celdas está vacío")
            else:
                logger.info(f"Usando {len(last_cells)} celdas para predicciones consolidadas")
                
                # GENERAR PREDICCIONES CONSOLIDADAS
                consolidated_predictions = []
                
                # Usar modelo avanzado si está disponible
                if args.use_advanced_models and advanced_predictor and len(cells_history) >= 2:
                    try:
                        # Entrenar con todo el historial
                        advanced_predictor.train(cells_history)
                        ...
                        for _, cell in last_cells.iterrows():
                            ...
                            track_id = cell['track_id']
                            track_history = []
                            for i, gdf in enumerate(cells_history):
                                if not gdf.empty and track_id in gdf['track_id'].values:
                                    track_info = gdf[gdf['track_id'] == track_id].iloc[0].copy()
                                    track_info['timestamp'] = timestamps[i]
                                    track_history.append(track_info)
                        ...
                        logger.error(f"Error generating advanced predictions for consolidated map: {e}")
                
                # Si no hay predicciones avanzadas, usar el modelo básico
                if not consolidated_predictions:
                    for _, cell in last_cells.iterrows():
                        track_id = cell['track_id']
                        if track_id in cell_tracker.tracked_cells:
                            track_history = cell_tracker.tracked_cells[track_id]
                            if len(track_history) >= cell_nowcaster.min_history_points:
                                for minutes in [15, 30, 45]:
                                    # Modificar el nowcaster para usar tiempos diferentes
                                    temp_nowcaster = FlashCellNowcaster(forecast_minutes=minutes)
                                    pred = temp_nowcaster._predict_cell_movement(track_history)
                                    if pred:
                                        pred['track_id'] = track_id
                ...
                # Crear y guardar el mapa consolidado
                if consolidated_predictions:
                    # Convertir a DataFrame
                    pred_df = pd.DataFrame(consolidated_predictions)
                    
                    # Añadir incertidumbre si está habilitada
                    if args.show_uncertainty and uncertainty_model and not pred_df.empty:
                        try:
                            pred_df = uncertainty_model.add_uncertainty_to_predictions(pred_df)
                        except Exception as e:
                            logger.error(f"Error adding uncertainty to consolidated predictions: {e}")
                    
                    # Crear geometrías
                    pred_gdf = cell_nowcaster.create_prediction_geometries(pred_df)
                    
                    # Generar mapa con todas las trayectorias e historial
                    consolidated_map = visualizer.create_track_visualization(
                        cells_history, 
                        timestamps, 
                        [pred_df]  # Lista de predicciones por tiempo
                    )
                    
                    # Guardar mapa consolidado
                    consolidated_map_file = 'consolidated_nowcast.html'
                    visualizer.save_interactive_map(consolidated_map, filename=consolidated_map_file)
                    logger.info(f"Consolidated map with tracks and predictions saved as {consolidated_map_file}")

    # Realizar evaluación completa si se solicita
    if args.evaluate and evaluator and len(all_tracked_gdfs) > 1 and len(all_predictions) > 1:
        try:
            logger.info("Performing detailed evaluation of nowcasting predictions...")
            
            # Preparar datos
            actual_cells = pd.concat(all_tracked_gdfs[1:])
            all_predictions_df = pd.concat(all_predictions[:-1])
            
            if not actual_cells.empty and not all_predictions_df.empty:
                # Calcular métricas
                metrics_df, aggregated_metrics = evaluator.calculate_prediction_metrics(
                    actual_cells, all_predictions_df
                )
                
                # Guardar métricas
                metrics_file = os.path.join(output_dir, 'evaluation', 'metrics.csv')
                metrics_df.to_csv(metrics_file, index=False)
                
                # Generar visualizaciones de errores
                if not metrics_df.empty:
                    evaluator.plot_error_distribution(
                        metrics_df,
                        save_path=os.path.join(output_dir, 'evaluation', 'error_distribution.png')
                    )
                    
                    error_map = evaluator.create_error_map(
                        metrics_df, actual_cells, all_predictions_df
                    )
                    
                    error_map_file = os.path.join(output_dir, 'evaluation', 'error_map.html')
                    error_map.save(error_map_file)
                    
                    logger.info(f"Evaluation results saved to {os.path.join(output_dir, 'evaluation')}")
                    
                    # Imprimir resumen de evaluación
                    print("\n=== Evaluación del Nowcasting ===")
